...
import type { OrderStatus, ReservationStatus, TableStatus } from "@ontime/web-shared";
...
const TABLE_TONE: Record<TableStatus, BadgeTone> = {
  free: "success",
  reserved: "warning",
  occupied: "danger",
};
...
export function TableStatusBadge({ status }: { status: TableStatus }) {
  return <Badge tone={TABLE_TONE[status]}>{status}</Badge>;
}
